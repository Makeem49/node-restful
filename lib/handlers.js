                    ...
                    _data.read("users", phone, (err, userData) => {
                        if (!err) {
                            ...
                            _data.delete("users", phone, (err) => {
                                if (!err) {
                                    // delete all checks linked to the user.
                                    
                                    // 1. get the user checks 
                                    const userChecks = typeof(userData.checks) == "object" && userData.checks instanceof Array ? userData.checks : [];
                                    const checkToDelete = userChecks.length;

                                    if (checkToDelete > 0) {
                                        let checkDeleted = 0;
                                        const deletedError = false 
                                        // 2. iterate and remove the user checks
                                            ...
                                            _data.delete("checks", check, (err) => {
                                                ...
                                                if (checkDeleted == checkToDelete) {
                                                    if(!deletedError) {
                                                        ...
                                                        callback(500, {"Error" : "Encounter error while deleting checks."})
                                                    }
                                                }
                                            })
                                        })
    ...
    if (id) {
        ...
        _data.read("checks", id, (err, checkData) => {
            if (!err) {
                ...
                if (tokenId) {
                    // verify the token 
                    handler._tokens.verifyToken(tokenId, checkData.userPhoneNumber, (isValid) => {
                        if (isValid) {
...
handler._checks.put = (data, callback) => {
    ...
    if (id) {
        // check if any a optional field is given , if not, return error 400
        
        if (protocol || url || method || successCode || timeoutSeconds) {
            // if any of the optional field is given, read the check 

            _data.read("checks", id, (err, checkData) => {
                if(!err) {
                    // if no error, update the check fields 
                    const tokenId = typeof(data.header.token) == "string" && data.header.token.trim().length == 20 ?  data.header.token.trim() : false
                    ...
                    handler._tokens.verifyToken(tokenId, checkData.userPhoneNumber, (isValid) => {
                        if (isValid) {
                            if (isValid) {
                                ...
                                // update the check in disk 
    
                                _data.update("checks", id, checkData, (err) => {
                                    if (!err) {
                                        callback(200, checkData)
                                    } else {
                                        callback(500, {"Error" : "can not update checks."})
                                    }
                                })
                            } else {
                                callback(403)
                    ...
                    callback(404, {"Error" : `Check with ${id} does not exist`})
                }
            })   
        } else {
            callback(403, {"Error" : "Missing require field."})
        }
...
handler._checks.delete = (data, callback) => {
    ...
    if (id) {
        // read the check if no error, else return error
        _data.read("checks", id, (err, checkData) => {
            if (!err) {
                // get the token header, and verify 
                const tokenId = typeof(data.header.token) == "string" && data.header.token.trim().length == 20 ?  data.header.token.trim() : false

                handler._tokens.verifyToken(tokenId, checkData.userPhoneNumber, (isValid) => {
                    if (isValid) {
                        // if token is valid, delete the check from file. 
                        _data.delete("checks", id, (err) => {
                            if(!err) {
                                // look up the user check and update it. 
                                _data.read("users", checkData.userPhoneNumber, (err, userData) =>{
                                    if (!err) {
                                        // remove the check id from the user check list 
                                        const userChecks = typeof(userData.checks) == "object" && userData.checks instanceof Array ? userData.checks : []
                                        const checkId = userChecks.indexOf(id)
                                        if (checkId > -1 ) {
                                            // if id check is included in userData.check, splice it out , else return error
                                            userChecks.splice(checkId, 1)
                                            ...
                                            callback(500, {"Error" : "Check is not associated with user, it can't be remove."})
                                        }
                                    } else {
                                        callback(500, {"Error" : "Could not find a user with the check."})
                                    }
                                })
                            } else {
                        ...
                        callback(403, {"Error" : "Invalid token or token expired"})
                    }
                })
            } else {
